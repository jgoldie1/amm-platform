...
export function isSupabaseConfigured() { return getConfig() !== null; }

async function request<T>(path: string, init: RequestInit, prefer?: string): Promise<T> {
  const config = getConfig();
  if (!config) throw new Error("SUPABASE_SERVER_NOT_CONFIGURED");
  ...
  if (!response.ok) throw new Error(`SUPABASE_REST_${response.status}: ${await response.text()}`);
  if (response.status === 204) return undefined as T;
  return (await response.json()) as T;
}

export async function insertRow<T extends JsonRecord>(table: string,row: JsonRecord): Promise<T> {
  const rows = await request<T[]>(table,{ method: "POST", body: JSON.stringify(row) },"return=representation");
  if (!rows[0]) throw new Error(`SUPABASE_INSERT_EMPTY:${table}`);
  return rows[0];
}
