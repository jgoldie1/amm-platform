...
export async function POST(request: Request) {
  ...
  if (requestedRole === 'host' && host.user_id !== user.id) return NextResponse.json({ error: 'host_permission_denied' }, { status: 403 });

  const participantLimit = ['debate','showcase','talent','karaoke','mic','vocal-box'].includes(mode) ? 20 : 12;
  if (requestedRole === 'host') await ensureLiveKitRoom(roomName, participantLimit);
  const grant = await mintLiveKitToken({ roomName, identity: user.id, displayName: user.email ?? user.id, canPublish: requestedRole === 'host', canSubscribe: true });
